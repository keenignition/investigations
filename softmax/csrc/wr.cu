...
__global__ void softmax_kernel_wr(const float* __restrict__ in, float* __restrict__ out, int M, int N) {
  extern __shared__ float smem[]; // we'll need smem to hold one value per warp ---> 256 threads / 32 = 8 warps = 8 floats
  int row = blockIdx.x;
  int tid = threadIdx.x;
  int laneId = tid % 32;
  int warpId = tid / 32;
  ...
  if (row < M) {
    ...
    // max within each group of 32 threads
    ...
    // localMax are stuck in the private registers of Threads 0, 32, 64, 96, 128, 160, 192, and 224
    if (laneId == 0) { smem[warpId] = localMax; }
    __syncthreads(); 
    ...
    // Warp-level reduction for the sum --- same as before
    for (int offset = 16; offset > 0; offset /= 2) {
        localSum += __shfl_down_sync(0xffffffff, localSum, offset);
    }
    if (laneId == 0) { smem[warpId] = localSum; }
    __syncthreads();
...
// ncu --kernel-name softmax_kernel_wr --launch-skip 5 --launch-count 1 --set full -o wr.ncu-rep ./wr
